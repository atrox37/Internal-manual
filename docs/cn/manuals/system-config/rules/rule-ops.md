...
规则操作页面用于管理规则条目本身（不含链路画布内容），是规则系统的基础信息维护入口。
通过本页面，您可以：

- **新增规则条目**：定义规则名称和描述，建立可编辑的规则实体
- **修改规则元信息**：按业务变化维护规则名称与说明文本
- **删除失效规则**：清理不再使用的规则，避免误用旧规则
- **为规则链路配置做准备**：先在本页建立规则，再进入规则链路页面配置执行流程
