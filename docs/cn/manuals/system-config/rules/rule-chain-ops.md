...
规则链路操作页面用于编排规则的执行流程与节点逻辑，是自动化策略落地的核心页面。该页面同时覆盖“查看执行状态”和“编辑流程画布”两类场景。
通过本页面，您可以：

- **查看链路详情与实时路径**：快速确认当前规则的执行分支和节点状态
- **可视化编辑规则画布**：通过拖拽卡片与连线构建完整执行链路
- **配置节点参数与判断逻辑**：支持 Switch Function、Change Value 等功能卡片
- **导入/导出 JSON 链路**：便于跨环境迁移、版本管理和备份恢复
- **全屏模式提升编辑效率**：在复杂链路下获得更清晰的画布操作体验
