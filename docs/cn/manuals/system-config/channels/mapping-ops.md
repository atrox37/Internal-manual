...
通道点位映射配置页面用于维护“业务点位”与“底层协议地址/寄存器”的映射关系，决定了数据采集和控制命令是否能正确落到设备上。
通过本页面，您可以：

- **按协议规则维护映射字段**：针对 modbus_rtu/modbus_tcp/di_do 等协议分别配置
- **批量修改并实时校验**：在编辑模式下统一调整 function code、data type、byte order 等关键项
- **通过 CSV 导入覆盖映射**：适合批量改造或项目迁移时快速更新配置
- **导出映射结果用于审计**：可将当前映射导出留档，便于交付和问题追溯
