...
实例点位路由配置页面用于建立“实例点位 -> 通道点位”的数据通路，决定实例数据如何与底层通道进行映射与交互。
通过本页面，您可以：

- **查看当前路由关系**：在 Routing 视图中检查 measurement/action 点位的映射目标
- **按规则批量修改路由**：支持编辑、校验、筛选（modified/invalid）后统一提交
- **通过 CSV 导入覆盖路由**：适合现场批量替换或环境迁移
- **导出路由配置做备份**：便于回滚、审计和跨项目复用
- **减少接线错误风险**：通过可视化校验提前发现点位类型或通道选择问题
